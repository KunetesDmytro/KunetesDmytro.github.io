...
// Використовуємо цикл while згідно з Варіантом 12 [cite: 1957]
function renderOrderHistory() {
  // Дані для замовлень (можна винести в глобальний стан)
  const historyData = [
    { id: '#PC-2024-041', date: '28 березня 2025', total: 1110, status: 'delivered' },
    { id: '#PC-2024-042', date: 'Сьогодні, 14:32', total: 570, status: 'cooking' }
  ];

  // Очищуємо контейнер перед рендерингом [cite: 1700, 1703]
  ordersListEl.innerHTML = '';

  let counter = 0; // Називаємо інакше, щоб уникнути Duplicate declaration
  while (counter < historyData.length) {
    const order = historyData[counter];
    ...
    orderCard.innerHTML = `
            <div class="order-header">
                ...
                <span class="order-status ${order.status}">
                    ${order.status === 'delivered' ? '✓ Доставлено' : '🔥 Готується'}
                </span>
            </div>
            <div class="order-footer">
                <span>Разом: <strong>₴ ${order.total}</strong></span>
            </div>
        `;
...
function placeOrder() {
  ...
  // Використовуємо let замість const, якщо імена можуть перетинатися,
  // або просто даємо унікальні імена для локальної функції
  const currentOrderId = `#PC-2025-0${orderIdCounter++}`;
  const orderTime = new Date();
  const timeLabel = `Сьогодні, ${orderTime.getHours()}:${String(orderTime.getMinutes()).padStart(2,'0')}`;

  // Розрахунок (прибираємо дублікати імен)
  const finalSubtotal = cart.reduce((sum, item) => sum + item.price * item.qty, 0);
  const finalDelivery = finalSubtotal >= 600 ? 0 : 50;
  const grandTotal = finalSubtotal + finalDelivery;

  // Створюємо картку замовлення [cite: 1707]
  const newOrderCard = document.createElement('article');
  newOrderCard.className = 'order-card status-cooking';
  newOrderCard.innerHTML = `
        <div class="order-header">
            <div>
                <span class="order-id">${currentOrderId}</span>
                <span class="order-date">${timeLabel}</span>
            </div>
            <span class="order-status cooking">🔥 Готується</span>
        </div>
        <div class="order-footer">
            <span>Разом: <strong>₴ ${grandTotal}</strong></span>
        </div>
    `;

  ordersListEl.insertBefore(newOrderCard, ordersListEl.firstChild);

  // Реалізація таймера очікування (Варіант 12)
  let minutesLeft = 60;
  const deliveryTimer = setInterval(() => {
    minutesLeft--;
    if (minutesLeft <= 0) {
      clearInterval(deliveryTimer);
      showToast("🍕 Ваша піца приїхала!");
    }
  }, 60000); // Оновлення раз на хвилину
  ...
  showToast(`✅ Замовлення ${currentOrderId} оформлено! Таймер: 60 хв.`);
}
